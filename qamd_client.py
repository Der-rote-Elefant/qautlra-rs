...
class QAMDClient:
    ...
    def __init__(self, 
                 ctp_url: str = "ws://localhost:8014/ws/market", 
                 qq_url: str = "ws://localhost:8016/ws/market",
                 sina_url: str = "ws://localhost:8012/ws/market"):
        """
        初始化行情客户端
...
async def main():
    ...
    try:
        # 订阅不同市场的行情数据
        if enable_ctp:
            await client.subscribe_ctp(["au2512", "IF2506", "rb2512"])
            await client.peek_ctp_subscriptions()
        
        if enable_qq:
            # QQ行情支持灵活的代码格式
            await client.subscribe_qq([ "SZSE.000001", "HKEX.00700"])
