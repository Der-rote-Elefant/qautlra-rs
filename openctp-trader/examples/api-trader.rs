...
const TRADER_FRONT: &'static str = "tcp://121.37.80.177:20004";
const BROKER_ID: &'static str = "4040";
const APPID: &'static str = "SHINNY_Q7V3_3.8";
const AUTH_CODE: &'static str = "XLRIQTTFTZ660E4K";
struct Spi;
impl TraderSpi for Spi {}

fn new_authenticate(user_id: &str) -> CThostFtdcReqAuthenticateField {
    ...
    set_cstr_from_str_truncate(&mut f.AppID, APPID);
    f
}
...
fn new_input_order(user_id: &str) -> CThostFtdcInputOrderField {
    ...
    set_cstr_from_str_truncate(&mut f.BrokerID, BROKER_ID);
    set_cstr_from_str_truncate(&mut f.InvestorID, user_id);
    set_cstr_from_str_truncate(&mut f.InstrumentID, "IF2406");
    set_cstr_from_str_truncate(&mut f.UserID, user_id);
    f.Direction = THOST_FTDC_D_Buy;
    f.OrderPriceType = THOST_FTDC_OPT_LimitPrice;
    f.LimitPrice = 3581.8 as f64;
...
fn main() {
    ...
    match trader_api.req_authenticate(&new_authenticate(&user_id), last_request_id) {
    ...
    match trader_api.req_qry_settlement_info_confirm(
        &new_qry_settlement_info_confirm(&user_id),
        last_request_id,
    ) {
    ...
    last_request_id += 1;
    match trader_api
        .req_settlement_info_confirm(&new_settlement_info_confirm(&user_id), last_request_id)
    {
    ...
    last_request_id += 1;
    match trader_api
        .req_qry_investor_position(&new_qry_investor_position(&user_id), last_request_id)
    {
