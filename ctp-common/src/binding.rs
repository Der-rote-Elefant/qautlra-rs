    ...
    fn fmt(&self, fmt: &mut fmt::Formatter) -> fmt::Result {
        fmt.debug_struct("CThostFtdcInstrumentField")
            ...
            .field(
                ...
                &char::from(self.MaxMarginSideAlgorithm),
            )
            .field(
                "UnderlyingInstrID",
                &gb18030_cstr_to_str(&self.UnderlyingInstrID),
            )
...
impl CThostFtdcDepthMarketDataField {
    pub fn to_struct(self) -> DepthMarketData {
        DepthMarketData {
            TradingDay: gb18030_cstr_to_str(&self.TradingDay).to_string(),
            InstrumentID: gb18030_cstr_to_str(&self.InstrumentID).to_string(),
            ExchangeID: gb18030_cstr_to_str(&self.ExchangeID).to_string(),
            ExchangeInstID: gb18030_cstr_to_str(&self.ExchangeInstID).to_string(),
            LastPrice: normalize_double(self.LastPrice)
                .or_else(|| Some(0.0))
                .unwrap(),
            PreSettlementPrice: normalize_double(self.PreSettlementPrice)
                .or_else(|| Some(0.0))
                .unwrap(),
            PreClosePrice: normalize_double(self.PreClosePrice)
                .or_else(|| Some(0.0))
                .unwrap(),
            PreOpenInterest: self.PreOpenInterest,
            OpenPrice: normalize_double(self.OpenPrice)
                .or_else(|| Some(0.0))
                .unwrap(),
            HighestPrice: normalize_double(self.HighestPrice)
                .or_else(|| Some(0.0))
                .unwrap(),
            LowestPrice: normalize_double(self.LowestPrice)
                .or_else(|| Some(0.0))
                .unwrap(),
            Volume: self.Volume,
            Turnover: self.Turnover,
            OpenInterest: self.OpenInterest,
            UpperLimitPrice: normalize_double(self.UpperLimitPrice)
                .or_else(|| Some(0.0))
                .unwrap(),
            LowerLimitPrice: normalize_double(self.LowerLimitPrice)
                .or_else(|| Some(0.0))
                .unwrap(),
            UpdateTime: gb18030_cstr_to_str(&self.UpdateTime).to_string(),
            UpdateMillisec: self.UpdateMillisec,
            BidPrice1: normalize_double(self.BidPrice1)
                .or_else(|| Some(0.0))
                .unwrap(),
            BidVolume1: self.BidVolume1,
            AskPrice1: normalize_double(self.AskPrice1)
                .or_else(|| Some(0.0))
                .unwrap(),
